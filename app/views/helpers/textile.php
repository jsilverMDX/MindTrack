<?php
// @(#) $Id: textile.php 19 2005-11-23 00:48:30Z wassimk $
...
';
  ...
  function process($str) {
    ...
    foreach ($para as $para) {
      if (preg_match('/^\n+$/s', $para)) {
        if (isset($sticky) && isset($stickybuff)) {
          $stickybuff .= $para;
        } else {
          $out .= $para;
        }
      ...
      if (preg_match('{^(h[1-6]|p|bq|bc|fn\d+)
        ...
        continue;
      } elseif (isset($sticky) && isset($stickybuff) &&
      ...
      if ($block == 'bc') {
        ...
        if ($sticky <= 1) {
          ...
          if (isset($padleft)) { $style .= ";padding-left:${padleft}em"; }
          if (isset($padright)) { $style .= ";padding-right:${padright}em"; }
          if (isset($clear)) { $style .= ";clear:${clear}"; }
          if (isset($class)) { $class = preg_replace('/^ /', '', $class, 1); }
          if (isset($class)) { $pre .= " class=\"$class\""; }
          if (isset($id)) { $pre .= " id=\"$id\""; }
          if (isset($style)) { $style = preg_replace('/^;/', '', $style, 1); }
          if (isset($style)) { $pre .= " style=\"$style\""; }
          if (isset($lang)) { $pre .= " lang=\"$lang\""; }
          if (isset($cite)) { $pre .= ' cite="' . $this->format_url(array('url' => $cite)) . '"'; }
          $pre .= '>';
          unset($clear);
        }
        $pre .= '<p>';
        ...
        $pre .= '<' . (isset($macros[$block]) ? $macros[$block] : $block);
        if (isset($align)) {
          $alignment = $this->_halign($align);
          if ($this->options['css_mode']) {
            if (($padleft || $padright) &&
        ...
        if (isset($style)) { $pre .= " style=\"$style\""; }
        if (isset($lang)) { $pre .= " lang=\"$lang\""; }
        if (isset($cite) && ($block == 'bq')) { $pre .= ' cite="' . $this->format_url(array('url' => $cite)) . '"'; }
        $pre .= '>';
        unset($clear);
      }
      ...
      if (preg_match('{' . $this->blocktags . '}x', $buffer)) {
        ...
        if (isset($filter)) { $buffer = $this->format_block(array('text' => "|$filter|" . $buffer, 'inline' => 1)); }
        $out .= $pre . $buffer . $post;
      }
    }
  ...
  function format_inline($args) {
    ...
    $text = preg_replace_callback('{(?:^|(?<=[\s>])|([{[]))      # $1: open brace/bracket
                                   ...
                                   }mx', $this->_cb('$me->_repl($me->repl[0], $me->format_image(array("pre" => $m[1], "src" => $m[5], "align" => ($m[2] ? $m[2] : $m[4]), "extra" => $m[6], "url" => isset($m[7]) ? $m[7] : null, "clsty" => $m[3], "post" => isset($m[8]) ? $m[8] : null)))'), $text);
    ...
    while ($redo) {
      ...
      foreach ($qtags as $tag) {
        ...
        if ($last != ($text = preg_replace_callback('{(?:^|(?<=[\s>\'"])|([{[]))                     # "\' $1 - pre
                                                     ...
                                                     }mx', $this->_cb('$me->format_tag(array("tag" => end($me->tmp["r"]), "marker" => end($me->tmp["f"]), "pre" => $m[1], "text" => $m[3], "clsty" => $m[2], "post" => isset($m[4]) ? $m[4] : null))'), $text))) {
          $redo = ($redo || ($last != $text));
          $last = $text;
        }
  ...
  function format_classstyle($clsty = NULL, $class = NULL, $style = NULL) {
    ...
    if (isset($id)) { $attrs .= " id=\"$id\""; }
    if (isset($style)) { $attrs .= " style=\"$style\""; }
    if (isset($lang)) { $attrs .= " lang=\"$lang\""; }
  ...
  function format_list($args) {
    ...
    foreach ($lines as $line) {
      if (preg_match('{^((?:' . $this->clstypadre . '*|' . $this->halignre . ')*)
        ...
        if (isset($itemclsty)) { $itemattr = $this->format_classstyle($itemclsty); }
        if ($depth > $last_depth) {
          for ($j = $last_depth; $j < $depth; $j++) {
            ...
            $out .= ">\n<li";
            if (isset($itemattr)) { $out .= " $itemattr"; }
            $out .= ">";
          }
  ...
  function format_image($args) {
    ...
    $pre = (isset($args['pre']) ? $args['pre'] : '');
    $post = (isset($args['post']) ? $args['post'] : '');
    $link = (isset($args['url']) ? $args['url'] : null);
    $clsty = $args['clsty'];
    $this->_strip_borders($pre, $post);
   ...
   $style = null;
    ...
    if (isset($w) && isset($h)) {
      ...
      if ((isset($image_w) && isset($image_h)) && (isset($w) || isset($h))) {
  ...
  function format_table($args) {
    ...
    foreach ($rows as $row) {
      ...
      for ($c = $colcount - 1; $c > 0; $c--) {
        ...
        $colstyle = '';
        $colalign = isset($colaligns[$c]) ? $colaligns[$c] : null;
        $col = array_pop($cols);
        $col = ($col ? $col : '');
        $attrs = '';
        ...
        if (isset($rowheader)) { $header = 1; }
        if (isset($header)) { $colaligns[$c] = $colalign; }
        $col = preg_replace('/^ +/', '', $col, 1); $col = preg_replace('/ +$/', '', $col, 1);
        if (strlen($col)) {
          // create one cell tag
          $rowspan = ($rowspans[$c] ? $rowspans[$c] : 0);
          $col_out = '<' . (isset($header) ? 'th' : 'td');
          if ($colalign) {
          ...
          if (isset($colpadl)) { $colstyle .= ";padding-left:${colpadl}em"; }
          if (isset($colpadr)) { $colstyle .= ";padding-right:${colpadr}em"; }
          if (isset($colclass)) { $col_out .= " class=\"$colclass\""; }
          if (isset($colid)) { $col_out .= " id=\"$colid\""; }
          if (isset($colstyle)) { $colstyle = preg_replace('/^;/', '', $colstyle, 1); }
          if (isset($colstyle)) { $col_out .= " style=\"$colstyle\""; }
          if (isset($collang)) { $col_out .= " lang=\"$collang\""; }
      ...
      if (isset($rowclass)) { $out .= " class=\"$rowclass\""; }
      if (isset($rowid)) { $out .= " id=\"$rowid\""; }
      if (isset($rowstyle)) { $out .= " style=\"$rowstyle\""; }
      $out .= ">$row_out</tr>";
    }
    ...
    if (isset($tpadl)) { $tstyle .= ";padding-left:${tpadl}em"; }
    if (isset($tpadr)) { $tstyle .= ";padding-right:${tpadr}em"; }
    if (isset($tclass)) { $tclass = preg_replace('/^ /', '', $tclass, 1); }
    if (isset($tclass)) { $table .= " class=\"$tclass\""; }
    if (isset($tid)) { $table .= " id=\"$tid\""; }
    if (isset($tstyle)) { $tstyle = preg_replace('/^;/', '', $tstyle, 1); }
    if (isset($tstyle)) { $table .= " style=\"$tstyle\""; }
    if (isset($tlang)) { $table .= " lang=\"$tlang\""; }
    if (isset($tclass) || isset($tid) || isset($tstyle)) { $table .= " cellspacing=\"0\""; }
    $table .= ">$out</table>";
...
/**
  ...
  function _cb($function) {  
    $current =& TextileHelper::_current_store($this);
    return create_function('$m', '$me =& TextileHelper::_current(); return ' . $function . ';');
  } // function _cb
 ...
 /* static */ function &_current() {
   $current =& TextileHelper::_current_store($null = NULL);
   return $current[0];
 } // function _current
} // class Textile
 ...
 */
class MTLikeTextile extends TextileHelper {
  /**
 ...
 * @version @(#) $Id: textile.php 19 2005-11-23 00:48:30Z wassimk $
 */
?>