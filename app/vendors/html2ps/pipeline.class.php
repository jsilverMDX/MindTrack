...
class Pipeline {
  ...
  function Pipeline() {
    ...
    $this->_DispatcherPdf =& new DispatcherPdf();

    $this->_DispatcherPdf->add_event('before-page-heights');
    $this->_DispatcherPdf->add_event('before-page');
    $this->_DispatcherPdf->add_event('after-page');
    $this->_DispatcherPdf->add_event('before-batch-item');
    $this->_DispatcherPdf->add_event('after-batch-item');
    $this->_DispatcherPdf->add_event('after-parse');
    $this->_DispatcherPdf->add_event('before-document');
    $this->_DispatcherPdf->add_event('after-document');
    $this->_DispatcherPdf->add_event('before-batch');
    $this->_DispatcherPdf->add_event('after-batch');
  ...
  function close() {
    $this->_DispatcherPdf->fire('after-batch', array('pipeline' => &$this));
...
//   }
  ...
  function &get_DispatcherPdf() {
    return $this->_DispatcherPdf;
  }
  ...
  function _process_item($data_id, &$media, $offset=0) {    
    $this->_DispatcherPdf->fire('before-batch-item', array('pipeline' => &$this));
    ...
    $this->_DispatcherPdf->fire('after-batch-item', array('pipeline' => &$this));
    return true;
  }
  ...
  function _show_item(&$box, $offset, &$context, &$media, &$postponed_filter) {
    ...
    $this->_DispatcherPdf->fire('before-page-heights', array('pipeline' => &$this, 
                                                          'document' => &$box,
                                                          'media' => &$media));
    ...
    $this->_DispatcherPdf->fire('before-document', array('pipeline' => &$this, 
                                                      'document' => &$box,
    ...
    for ($i = 0; $i < $expected_pages; $i++) {
      ...
      $this->_DispatcherPdf->fire('before-page', array('pipeline' => &$this, 
                                                    'document' => &$box, 
                                                    'pageno' => $i));
      ...
      $this->_DispatcherPdf->fire('after-page', array('pipeline' => &$this, 
                                                   'document' => &$box, 
                                                   'pageno' => $i));
    };

    $this->_DispatcherPdf->fire('after-document', array('pipeline' => &$this, 
                                                     'document' => &$box));
  }
  ...
  function process_batch($data_id_array, &$media) {
    ...
    $this->_DispatcherPdf->fire('before-batch', array('pipeline' => &$this));
  ...
  function &_layout_item($data_id, &$media, $offset, &$context, &$postponed_filter) {
    ...
    $this->_DispatcherPdf->fire('after-parse', array('pipeline' => &$this,
                                                  'document' => &$box,
                                                  'media' => $media));
